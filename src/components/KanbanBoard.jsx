...
import { Plus, Edit2, Trash2, Search, X, GripVertical, Palette } from 'lucide-react';
          ...
          {quadros.map((quadro) => {
            ...
            return (
              <motion.div 
                ...
                key={quadro.id} className="flex flex-col min-w-[320px] w-[320px]"
              >
                        ...
                        {quadroCards.map((card, index) => {
                          ...
                          return (
                            <Draggable key={card.id} draggableId={card.id} index={index} isDragDisabled={!canEdit}>
                              {(provided, snapshot) => (
                                <div
                                  ref={provided.innerRef} {...provided.draggableProps} onClick={() => onViewCard(card)} 
                                  className={`group bg-white dark:bg-igs-panel p-4 mb-3 rounded-2xl border-l-4 cursor-pointer transition-all ${leftBorder} ${
                                    snapshot.isDragging ? 'shadow-2xl scale-105 opacity-95 ring-2 ring-igs-primary z-50' : 'shadow hover:shadow-lg border-y border-r border-slate-100 dark:border-slate-800'
                                  }`}
