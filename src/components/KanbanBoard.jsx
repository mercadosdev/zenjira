...
import { Plus, Edit2, Trash2, Search, X, GripVertical, Palette, TextCursorInput, Copy, ArrowRight, ChevronUp, ChevronDown, Calendar, History, Flame, MessageSquare } from 'lucide-react';
            ...
            {quadros.map((quadro) => {
              ...
              return (
                <motion.div 
                  ...
                  key={quadro.id} 
                  className={`flex flex-col shrink-0 min-w-[88vw] sm:min-w-[300px] w-[88vw] sm:w-[300px] snap-center ${isColFocused ? 'relative z-[120]' : 'z-10'}`}
                >
                  <div className={`p-3 rounded-t-2xl border-b border-slate-200 dark:border-slate-800 border-t-4 shadow-sm flex flex-col gap-2 relative transition-colors ${boardTheme}`}>
                    <div className="flex justify-between items-center">
                      <h3 className="font-bold text-sm text-slate-800 dark:text-white m-0 flex items-center gap-2">
                        {quadro.name}
                        <span className="bg-white/50 dark:bg-black/20 text-slate-800 dark:text-slate-200 text-[10px] font-semibold px-2 py-0.5 rounded-full shadow-sm">
                          {quadroCards.length}
                        </span>
                      </h3>
                      
                      <div className="flex items-center gap-1 text-slate-400 dark:text-slate-500">
                        <button onClick={() => toggleSearch(quadro.id)} className="p-1 hover:text-blue-500 hover:bg-white/40 dark:hover:bg-slate-900/40 rounded-lg transition-all"><Search size={14} /></button>
                        
                        {isIgs && (
                          <>
                            <div className="relative">
                              <button onClick={() => setColorPickerOpen(colorPickerOpen === quadro.id ? null : quadro.id)} className="p-1 hover:text-amber-500 hover:bg-white/40 dark:hover:bg-slate-900/40 rounded-lg transition-all"><Palette size={14} /></button>
                            ...
                            <button onClick={() => onRenameQuadro(quadro.id, quadro.name)} className="p-1 hover:text-blue-500 hover:bg-white/40 dark:hover:bg-slate-900/40 rounded-lg transition-all"><Edit2 size={14} /></button>
                            <button onClick={() => onDeleteQuadro(quadro.id, quadro.name)} className="p-1 hover:text-red-500 hover:bg-white/40 dark:hover:bg-slate-900/40 rounded-lg transition-all"><Trash2 size={14} /></button>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    {showSearch[quadro.id] && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="relative mt-1">
                        <input 
                          ...
                          className="w-full pl-7 pr-7 py-1.5 text-xs rounded-xl border-none bg-white/60 dark:bg-slate-900/60 backdrop-blur-md text-slate-800 dark:text-slate-200 outline-none focus:ring-2 focus:ring-igs-primary transition-all shadow-inner"
                        />
                        <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-500" />
                        {colSearch[quadro.id] && <button onClick={() => handleSearchChange(quadro.id, '')} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-500 hover:text-red-500"><X size={12} /></button>}
                      </motion.div>
                    )}
                  </div>
                  ...
                  <Droppable droppableId={quadro.id} isDropDisabled={!canEdit}>
                    {(provided, snapshot) => (
                      <div 
                        ...
                        className={`flex-1 p-2 rounded-b-2xl transition-colors duration-200 min-h-[150px] shadow-sm border border-t-0 border-slate-200 dark:border-slate-800 ${
                      ...
                      >
                                ...
                                {(provided, snapshot) => {
                                  ...
                                  return (
                                    <div
                                      ...
                                      onMouseEnter={(e) => handleMouseEnter(e, card.id, hasTooltip)}
                                    ...
                                    >
                                      ...
                                      <div className="flex items-start justify-between gap-2 mb-2">
                                        <div className="flex-1">
                                          {cardData?.envioPrioritario && (
                                            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded border border-red-200 dark:border-red-900/60 text-[8px] font-bold tracking-widest text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 mb-1.5">
                                              <Flame size={10}/> URGENTE
                                            </span>
                                          )}
                                          <h4 className="font-semibold text-slate-800 dark:text-slate-100 text-xs break-words leading-snug">
                                        ...
                                        {canEdit && (
                                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <div className="flex flex-col border border-slate-200 dark:border-slate-700 rounded-md bg-slate-50 dark:bg-slate-900/50 overflow-hidden">
                                              <button onClick={(e) => { e.stopPropagation(); moveCardManual(card.id, quadro.id, 'up'); }} className="p-px text-slate-400 hover:text-igs-primary hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors" title="Mover para cima"><ChevronUp size={12}/></button>
                                              <div className="h-px bg-slate-200 dark:bg-slate-700 w-full"></div>
                                              <button onClick={(e) => { e.stopPropagation(); moveCardManual(card.id, quadro.id, 'down'); }} className="p-px text-slate-400 hover:text-igs-primary hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors" title="Mover para baixo"><ChevronDown size={12}/></button>
                                            </div>
                                            <div {...provided.dragHandleProps} className="p-0.5 text-slate-300 dark:text-slate-600 hover:text-igs-primary cursor-grab active:cursor-grabbing" onClick={e => e.stopPropagation()}>
                                      ...
                                      {cardData?.previsaoEntrega && (
                                        <div className="flex items-center gap-1.5 mb-2">
                                          <span className="flex items-center gap-1 text-[9px] font-semibold text-slate-500 dark:text-slate-400">
                                            <Calendar size={10}/> {new Date(cardData.previsaoEntrega + 'T12:00:00').toLocaleDateString()}
                                          </span>
                                          {cardData?.adiamentosEntrega > 0 && (
                                            <span className="flex items-center gap-0.5 text-[8px] font-bold text-orange-500 bg-orange-50 dark:bg-orange-900/20 px-1 rounded" title={`${cardData.adiamentosEntrega} adiamentos registrados`}>
                                              <History size={8}/> +{cardData.adiamentosEntrega}
                                            </span>
                                          )}
                                        </div>
                                      )}
                                      ...
                                      <div className="flex justify-between items-center mt-2.5 pt-2.5 border-t border-slate-50 dark:border-slate-800/50">
                                        <span className="text-[10px] truncate max-w-[80px] font-medium text-slate-500 dark:text-slate-400" title={cardData?.responsavel}>{cardData?.responsavel || 'Sem dono'}</span>
                                        {isIgs && cardData?.jira && (
                                          <span className="inline-block bg-igs-primary/10 text-igs-primary border border-igs-primary/20 text-[8px] font-semibold px-1.5 py-px rounded truncate max-w-[70px]">
                                            Jira: {cardData.jira}
                                          </span>
                                        )}
                                      </div>
                        ...
                        {canEdit && (
                          <button 
                            ...
                            className="w-full mt-1.5 py-2 flex items-center justify-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-igs-primary dark:hover:text-igs-accent hover:bg-white dark:hover:bg-slate-800 rounded-lg transition-all border border-transparent hover:border-slate-200 dark:hover:border-slate-700 shadow-sm"
                          >
                            <Plus size={14} /> {t(language, 'newTask')}
                          </button>
                        )}
                      </div>
                    )}
          ...
          {isIgs && (
            <div className="min-w-[320px] w-[320px] flex flex-col shrink-0 snap-center">
              <button 
                ...
                className="h-12 flex items-center justify-center gap-2 bg-slate-200/50 dark:bg-slate-800/50 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-600 font-semibold transition-colors"
              >
