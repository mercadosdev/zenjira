...
export default function FilterBar({ searchTerm, setSearchTerm, filters, setFilters, quadros }) {
  ...
  // Estado para controlar a expansão dos filtros no Mobile
  ...
  // Verifica se há algum filtro ativo (além da busca) para destacar o botão no mobile
  ...
  return (
    <div className="bg-slate-50 dark:bg-igs-panel p-3 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row flex-wrap gap-2 md:gap-3 md:items-center shadow-sm relative z-[150]">
      
      {/* LINHA 1 (Mobile): Busca + Botão de Expandir Filtros */}
      <div className="flex w-full md:w-auto md:flex-1 gap-2">
        ...
        {/* Botão Toggle (Aparece apenas em md:hidden) */}
        <button 
          onClick={() => setIsExpanded(!isExpanded)}
          className={`md:hidden px-3 py-2 rounded-xl transition-colors flex items-center justify-center shrink-0 border ${
            isExpanded || hasActiveFilters 
              ? 'bg-igs-primary border-igs-primary text-white shadow-sm' 
      ...
      {/* LINHA 2 (Mobile) / Continuação (Desktop): Os Filtros */}
      {/* Usamos 'hidden md:flex' para esconder no mobile, mas forçamos 'flex' se isExpanded for true */}
      <div className={`w-full md:w-auto flex-col sm:flex-row flex-wrap gap-2 md:gap-3 flex-1 ${isExpanded ? 'flex' : 'hidden md:flex'}`}>
        
        <div className="flex-1 min-w-[120px] md:max-w-[160px] z-[60]">
          <CustomSelect value={filters.quadroId || ''} onChange={(val) => updateFilter('quadroId', val)} options={quadroOptions} />
        </div>

        <div className="flex-1 min-w-[120px] md:max-w-[170px] z-[50]">
          <CustomSelect value={filters.responsavel || ''} onChange={(val) => updateFilter('responsavel', val)} options={respOptions} />
        </div>

        <div className="flex-1 min-w-[110px] md:max-w-[140px] z-[40]">
          <CustomSelect value={filters.status || ''} onChange={(val) => updateFilter('status', val)} options={statusOptions} colorMap="status" />
        </div>
        
        <div className="flex-1 min-w-[110px] md:max-w-[140px] z-[30]">
          <CustomSelect value={filters.categoria || ''} onChange={(val) => updateFilter('categoria', val)} options={catOptions} colorMap="category" />
        </div>

        {isIgs && (
          <div className="flex-1 min-w-[100px] md:max-w-[120px] z-[20]">
        ...
        <button 
          onClick={() => { 
            setSearchTerm(''); 
            setFilters({ responsavel: '', status: '', categoria: '', tipo: '', quadroId: '' }); 
            setIsExpanded(false); // Fecha o painel no mobile ao limpar
          }}
          className="shrink-0 w-full sm:w-auto px-3 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl transition-colors text-xs font-bold flex items-center justify-center gap-1.5 mt-1 sm:mt-0"
        >
