...
export default function FilterBar({ searchTerm, setSearchTerm, filters, setFilters, quadros }) {
  ...
  // Arrays de Opções para os Componentes Customizados
  ...
  const prioOptions = [{ value: '', label: t(language, 'priority') }, { value: 'Baixa', label: t(language, 'Baixa') }, { value: 'Média', label: t(language, 'Média') }, { value: 'Alta', label: t(language, 'Alta') }];
  const compOptions = [{ value: '', label: t(language, 'complexity') }, { value: 'Baixa', label: t(language, 'Baixa') }, { value: 'Média', label: t(language, 'Média') }, { value: 'Alta', label: t(language, 'Alta') }];
  ...
  return (
    <div className="bg-slate-50 dark:bg-igs-panel p-4 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-wrap gap-3 md:gap-4 items-center shadow-sm relative z-[150]">
      ...
      {isIgs && (
        <>
          <div className="w-[calc(50%-6px)] sm:w-36 z-[40]">
            <CustomSelect value={filters.prioridade || ''} onChange={(val) => updateFilter('prioridade', val)} options={prioOptions} />
          </div>
          <div className="w-[calc(50%-6px)] sm:w-40 z-[30]">
            <CustomSelect value={filters.complexidade || ''} onChange={(val) => updateFilter('complexidade', val)} options={compOptions} />
          </div>
