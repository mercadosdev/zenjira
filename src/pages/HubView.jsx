...
import { FolderKanban, Sun, Moon, Kanban, Table, History, ChevronLeft, Copy, Check, Settings, Languages, RefreshCw, Zap, Users } from 'lucide-react';
...
// IMPORTAÇÃO DO NOVO AVATAR AQUI
...
export default function HubView() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-igs-bg dark:bg-igs-dark text-slate-800 dark:text-slate-200 transition-colors duration-300">
      ...
      <aside className="w-64 bg-white dark:bg-igs-panelDark flex flex-col flex-shrink-0 shadow-lg dark:shadow-2xl z-20 border-r border-slate-200 dark:border-slate-800/50 transition-colors duration-300">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-20 bg-white dark:bg-igs-panel border-b border-slate-200 dark:border-slate-800 px-6 lg:px-8 flex justify-between items-center z-10 shadow-sm transition-colors duration-300">
          <div className="flex items-center gap-6">
            <div>
              <h1 className="text-xl lg:text-2xl font-extrabold text-slate-800 dark:text-white m-0 tracking-tight flex items-center gap-3">
                {activeHub.name}
                {isIgs && (
              ...
              <span className="text-[10px] lg:text-xs font-bold uppercase tracking-wider mt-1 flex gap-1">
            ...
            {/* AQUI OS AVATARES SUBSTITUÍDOS E INTEGRADOS */}
            <div className="hidden md:flex items-center gap-1.5 pl-6 border-l border-slate-200 dark:border-slate-700">
          ...
          <div className="flex items-center gap-2 lg:gap-4">
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
              {(isIgs || isClientEditor) && (
                <button onClick={() => setIsQuickAddOpen(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-xs font-black shadow-sm transition-colors" title="Adição Rápida">
                  <Zap size={14} className="fill-white" />
                </button>
              )}
              {isIgs && (
                <button onClick={() => setIsTeamModalOpen(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-slate-700 text-igs-primary dark:text-white rounded-lg text-xs font-black shadow-sm transition-colors hover:text-blue-500" title="Status da Equipe">
                  <Users size={14} />
                </button>
              )}
            </div>
            ...
            <button onClick={handleManualRefresh} className={`p-2 text-slate-400 hover:text-blue-500 transition-colors ${isRefreshing ? 'animate-spin text-blue-500' : ''}`} title={t(language, 'refresh')}>
              <RefreshCw size={20} />
            </button>
            <button onClick={toggleLanguage} className="flex items-center gap-1 p-2 text-slate-400 hover:text-igs-primary dark:text-slate-400 font-bold text-xs">
        ...
        <main className="flex-1 overflow-x-auto overflow-y-auto p-6 md:p-8 custom-scrollbar relative">
          {currentView !== 'kanban' && (
            <div className="mb-6">
